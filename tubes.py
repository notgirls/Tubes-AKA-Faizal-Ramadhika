...
# Fungsi rekursif untuk simulasi proses pasien
def process_patients_recursive(n, process_time=0.01):
    if n <= 0:
        return
    # Simulasi overhead stack call
    time.sleep(process_time)  # Simulasi waktu proses
    process_patients_recursive(n - 1, process_time * 1.2)  # Tambah overhead

# Fungsi iteratif untuk simulasi proses pasien
def process_patients_iterative(n, process_time=0.01):
    for _ in range(n):
        time.sleep(process_time)  # Simulasi waktu proses
...
while True:
    try:
        n = int(input("Masukkan jumlah pasien (n) (atau ketik -1 untuk keluar): "))
        if n == -1:
        ...
        print(f"Memulai proses rekursif untuk {n} pasien...")
        start_time = time.time()
        process_patients_recursive(n)
        recursive_time = time.time() - start_time
        recursive_times.append(recursive_time)
        print(f"Waktu eksekusi rekursif: {recursive_time:.4f} detik")

        # Ukur waktu eksekusi algoritma iteratif
        print(f"Memulai proses iteratif untuk {n} pasien...")
        start_time = time.time()
        process_patients_iterative(n)
        ...
        print("Masukkan jumlah pasien (n) yang valid!")
